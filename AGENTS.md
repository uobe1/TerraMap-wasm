...
- 导出核心模块：`colors`, `data_stream`, `world_loader`, `renderer`, `search`
...
- `read_bool()`, `read_string()` - 读取布尔值和字符串（使用 7-bit 变长编码）
- `skip()`, `seek()`, `position()` - 流操作

**重要特性**：
- `read_string()` 使用 7-bit 可变长度整数编码读取字符串长度，与原版 JavaScript 完全一致
- 支持变长字符串的高效解析
...
**关键修复**：
- 正确实现重要性位图读取（每 7 个位存储在一个字节中）
- 使用 7-bit 编码解析变长字符串
- 支持 Terraria 1.4.5 版本格式（版本号 279）
...
当前版本支持 Terraria 1.4.5 世界文件（文件格式版本 279）。
